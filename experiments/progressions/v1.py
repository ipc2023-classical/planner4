...
BUILDS = ["ipc23"]
CONFIG_NICKS = [
    (f"rhw-goals={goals}-gn={gn}-reas={reas}", [
        "--search",
        "let(hlm,"
        "landmark_sum(lm_factory=lm_reasonable_orders_hps(lm_rhw()),"
        f"transform=adapt_costs(one), prog_goal={goals},prog_gn={gn},"
        f"prog_r={reas}), lazy_greedy([hlm], cost_type=one,"
        "reopen_closed=false))"
    ])
    for goals in [True, False]
    for gn in [True, False]
    for reas in [True, False]
]
CONFIGS = [
    IssueConfig(
        config_nick,
