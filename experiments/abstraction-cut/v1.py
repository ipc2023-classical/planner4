...
ARCHIVE_PATH = "buechner/ipc23-landmarks/abstraction-cut"
DIR = os.path.dirname(os.path.abspath(__file__))
BENCHMARKS_DIR = os.environ["DOWNWARD_BENCHMARKS"]
REVISIONS = ["676e2bcb3"]
BUILDS = ["ipc23"]
CONFIG_NICKS = [
    (f"abstraction-cut-sum", [
        "--search",
        "let(hlm, dalm_sum(abstraction_cut(), "
        "transform=adapt_costs(one)), "
        "lazy_greedy([hlm], cost_type=one, reopen_closed=false))"
    ]),
    (f"abstraction-cut-hs", [
        "--search",
        "let(hlm, dalm_greedy_hs(abstraction_cut(), "
        "transform=adapt_costs(one)), "
        "lazy_greedy([hlm], cost_type=one, reopen_closed=false))"
    ]),
]
